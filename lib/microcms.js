...
const fallbackTeam = {
  teamName: '〇〇少年野球クラブ',
  teamLabel: 'YOUR TEAM',
  motto: '楽しく、真剣に、仲間とともに',
  subtitle: '野球を通じて子どもたちの成長を応援します',
...
export async function getReportById(id) {
  ...
  const data = await client.get({
    ...
    contentId: id,
    customRequestInit: {
      next: { tags: [CACHE_TAGS.reports, CACHE_TAGS.report(id)] },
    },
  });
...
export async function getTeamInfo() {
  ...
  try {
    ...
    return {
      ...
      teamLabel: data.teamLabel || 'YOUR TEAM',
      motto: data.motto || '',
