...
export default function HomeContent({ reports, team, players, coaches }) {
  ...
  const filteredMembers = activeMemberGrade === 'all' ? players : players.filter((m) => m.grade === activeMemberGrade);
  const grades = ['all', ...Array.from(new Set(players.map((m) => m.grade)))];
  ...
  return (
    <>
      ...
      <section id="top" className="fade-up" style={{
        ...
        {team.mainVisual ? (
          <div className="fade-up-1" style={{ width: '300px', height: '190px', borderRadius: '16px', overflow: 'hidden', marginBottom: '28px', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            <img src={team.mainVisual} alt={team.teamName} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          </div>
        ) : (
      ...
      <section style={{ padding: '0 20px', marginTop: '-28px', position: 'relative', zIndex: 10 }}>
        <div style={{ maxWidth: '600px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          {[
            { icon: '🕐', label: '活動日時', value: team.schedule || '毎週 土・日\n9:00〜12:00', accent: '#0066FF' },
            { icon: '📍', label: '活動場所', value: team.location || '未設定', accent: '#00C853' },
            { icon: '👦', label: '対象', value: team.target || '小学\n1年生〜6年生', accent: '#FF9100' },
            { icon: '📞', label: '連絡先', value: team.contactInfo || '未設定', accent: '#FF1744' },
          ].map((item, i) => (
      ...
      <section style={{ padding: '48px 20px', maxWidth: '600px', margin: '0 auto' }}>
        ...
        <div style={{ background: '#fff', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 16px rgba(0,0,0,0.05)', position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontSize: '14px', color: '#444', lineHeight: 2.1, whiteSpace: 'pre-wrap' }}>
            {team.introduction || '〇〇少年野球クラブは、2015年に設立された少年野球チームです。「楽しく、真剣に、仲間とともに」をモットーに、野球の技術だけでなく、礼儀やチームワークの大切さを学んでいます。'}
          </div>
        </div>
      </section>
            ...
            {filteredReports.map((report, i) => {
              ...
              return (
                <Link key={report.id} href={`/reports/${report.id}`} className="report-card" style={{ background: '#fff', borderRadius: '14px', overflow: 'hidden', display: 'flex', cursor: 'pointer', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', animation: `fadeInUp 0.5s ease ${i * 0.1}s both` }}>
                  {report.thumbnail ? (
                    <div style={{ width: '110px', minHeight: '110px', flexShrink: 0, position: 'relative' }}>
                  ...
                  <div style={{ padding: '14px 16px', flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#888', margin: 0, lineHeight: 1.7, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{preview}</p>
                  </div>
                </Link>
              );
            })}
              ...
              {coaches.map((coach, i) => (
                <div key={coach.id} className="coach-card" style={{ background: '#fff', borderRadius: '14px', overflow: 'hidden', display: 'flex', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', animation: `fadeInUp 0.5s ease ${i * 0.1}s both` }}>
                  ...
                  <div style={{ padding: '14px', flex: 1 }}>
                    <div style={{ fontSize: '14px', fontWeight: 900, color: '#222', marginBottom: '4px' }}>{coach.name}</div>
